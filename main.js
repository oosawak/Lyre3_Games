...
function updateAllUITexts() {
  ...
  setTextAll('#workshop-panel .workshop-tabs .workshop-tab', ['workshop_tab_theme', 'workshop_tab_vrm', 'workshop_tab_glb', 'workshop_tab_game_mode', 'workshop_tab_apps']);
...
function omitScaleSettings(value) {
  if (Array.isArray(value)) {
    return value.map(omitScaleSettings);
  }
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value)
        .filter(([key]) => key !== 'scale')
        .map(([key, entry]) => [key, omitScaleSettings(entry)])
    );
  }
  return value;
}
...
vrmCopyBtn.addEventListener('click', () => {
  copySettingsToClipboard('VRM', omitScaleSettings(vrmSettings), vrmCopyBtn);
});
...
glbCopyBtn.addEventListener('click', () => {
  copySettingsToClipboard('GLB', omitScaleSettings({
    model: glbSettings,
...
const workshopDetailOpen = document.getElementById('workshop-detail-open');
...
function getGameModeMockUrl() {
  return window.location.pathname.includes('/docs/')
    ? './game001/index.html'
    : '../game-mode/game001/index.html';
}
...
async function showWorkshopDetail(id) {
  try {
    ...
    if (workshopDetailOpen) {
      workshopDetailOpen.classList.toggle('hidden', item.item_type !== 'game_modes');
    }
...
workshopDetailOpen?.addEventListener('click', () => {
  if (!workshopCurrentItem || workshopCurrentItem.item_type !== 'game_modes') return;
  window.open(getGameModeMockUrl(), '_blank', 'noopener,noreferrer');
});
...
workshopDetailApply.addEventListener('change', async () => {
  ...
  if (workshopDetailApply.checked) {
    try {
      ...
      } else if (item.item_type === 'game_modes') {
        localStorage.setItem('active_game_mode', item.id);
      }
